...
def get_min_price_by_ruten():
    ...
    title = (re.sub(r"【.*?】", "", title).strip().replace("台灣現貨", "")).replace(
        "台灣出貨", ""
    )
...
def upload_to_ruten():
    ...
    if platform == "1688" or platform == "ruten":
        ...
        detail_img_list_new = [
            "https:" + img if "https:" not in img else img for img in detail_img_list
        ]
    ...
    # 创建 RutenUpload 的实例
    ruten_uplaod_instance = RutenUpload(
        store,
        is_add_main_logo=is_add_main_logo,
    ...
    if not category_id_status:
        ...
        return {
            ...
            "store": store,
            "product_id": product_id,
            "data": f"{category_id}-类目id不合法(请更换)",
        }

    logging.info(f"{product_id}-{category_id}-类目id验证可用")
    # 进行价格转换
    product_data = ruten_uplaod_instance.price_conversion(
        product_data, price_multi, price_add
    )
    ...
    if platform == "1688" or platform == "pinduoduo":
        detail_html = ruten_uplaod_instance.construction_details_html(
            product_data["data"]["details_text_description"],
            product_data["data"]["detailed_picture"],
        )
    elif platform == "ruten":
        ...
        detail_html = ruten_uplaod_instance.construction_details_html(
            product_data["data"]["details_text_description"], detail_img_list_new
        )
    ...
    upload_products = ruten_uplaod_instance.upload_products(
        product_id, upload_product_package
    )
    if upload_products["code"] == 0 or upload_products["code"] == 4:
        ...
        return {
            ...
            "store": store,
            "product_id": product_id,
            "after_listing_id": upload_products["after_listing_id"],
            "data": "上架成功",
        }
