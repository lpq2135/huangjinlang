...
# 文件日志
logging.basicConfig(
    filename=os.path.join(log_dir, "mylog.log"), format=log_format, level=level
)
...
def get_shop_by_mysql():
    ...
    try:
        cursor.execute(
            "SELECT store FROM exp_automatically_down WHERE account = %s AND operation_time != %s",
            (
                account,
                current_date,
            ),
...
def writing_to_database(store):
    ...
    try:
        cursor.execute(
            "UPDATE exp_automatically_down SET operation_time = %s WHERE account = %s AND store = %s;",
    ...
    for _ in range(max_retries):
        try:
            response = requests.get(
                url=url, headers=header, proxies=proxies, timeout=15
            ).text
            ...
            if product_num is not None:
                ...
                for i in id_list:
                    product_data = {
                        ...
                        "stock": tree.xpath(f'//tr[@data-gno="{i}"]/td[5]/text()')[0]
                        .replace("(", "")
                        .strip(),
...
def get_the_number_of_online_products(store, cookie):
    ...
    for _ in range(max_retries):
        try:
            ...
            if product_num is not None:
                total_num = (
                    tree.xpath('//div[@class="rt-text-activated"]/text()')[0]
                    .split("/")[1]
                    .replace(",", "")
                )
    ...
    for _ in range(max_retries):
        try:
            response = requests.post(
                url=url, headers=header, data=data, proxies=proxies, timeout=15
            )
            response.raise_for_status()
            if (
                response.json()["status"] == "success"
                or response.json()["status"] == "fail"
            ):
...
def process_store(store):
    ...
    if int(product_num1) <= int(total_num1) * float(delisting_percentage):
        ...
        page = (
            400
            if math.ceil(int(product_num1) / 30) >= 400
            else math.ceil(int(product_num1) / 30)
        )
        while True:
            ...
            if int(product_num2) >= int(total_num1) * float(delisting_percentage):
                gno_list1 = ",".join(
                    [
                        ...
                        for item in list(
                            filter(
                                lambda x: int(x["price"]) < int(price)
                                and int(x["sales"]) < int(bid)
                                or int(x["price"]) >= int(price)
                                and int(x["sales"]) < int(bid)
                                and int(x["click"]) < int(click)
                                and int(x["cart"]) < int(cart),
                                new_list2,
                            )
                        )
                    ]
                ...
                if gno_list1:
                    logging.info(
                        f"{store} 当前产品数:{product_num2}，当前页数：{page}，符合条件的商品数：{number_of_ids}"
                    )
                    if remove_products(store, cookie, gno_list1):
                        if page > 1:
                            ...
                            page = (
                                400
                                if math.ceil(int(product_num2) / 30) >= 400
                                else math.ceil(int(product_num2) / 30)
                            )
                    else:
                        time.sleep(10)
                else:
...
if user_authentication():
    first_row = pd.read_excel(
        r"C:\Users\Administrator\Desktop\扩充包自动下架\参数配置表.xlsx"
    ).iloc
    ...
    while True:
        ...
        if not stores:
            ...
            threads = [
                threading.Thread(target=process_store, args=(store,))
                for store in stores
            ]
