...
class BaseCrawler:
    ...
    def request_function(
        ...
        for attempt in range(max_retries):
            try:
                ...
                # 单独处理超时错误
                logging.warning(
                    f"请求超时 ({method}-{url}): {e}. 重试中... ({attempt + 1}/{max_retries})"
                )
                ...
                if hasattr(e, "response") and e.response is not None:
                    if e.response.status_code in (400, 401, 404):
                        return e.response
                logging.warning(
                    f"请求错误 ({method}-{url}): {e}. 重试中... ({attempt + 1}/{max_retries})"
                )
                ...
                # 处理其他未知异常
                logging.warning(
                    f"未知错误 ({method}-{url}): {e}. 重试中... ({attempt + 1}/{max_retries})"
                )
                time.sleep(2**attempt)
        ...
        # 双规格处理
            ...
            for parent_id, children in structure.items():
                for child_id in children.keys():
                    ...
                    sku_item = next(
                        (i for i in sku_parameters if i["name"] == combined_name), None
                    )
                    ...
                    specs[child_id] = {
                        ...
                        "spec_num": (
                            sku_item["stock"]
                            if sku_item and 100 <= total_stock <= 9998
                            else 249
                        ),
    ...
    def zh_to_tw(self, data):
        ...
        if data["data"]["specifications"] != 0:
            # 属性名转换
            sku_data["sku_property_name"] = {
                k: converter.convert(v)
                for k, v in sku_data["sku_property_name"].items()
            }
            # 参数列表转换
        ...
        if isinstance(data["data"]["details_text_description"], list):
            data["data"]["details_text_description"] = [
                converter.convert(i) for i in data["data"]["details_text_description"]
            ]
        else:
            data["data"]["details_text_description"] = converter.convert(
                data["data"]["details_text_description"]
            )
        return data
    ...
    def construction_details_html(self, detail_text, detail_img):
        ...
        if detail_img:
            html_img = "".join(
                f'<img alt="" src="{url}" width="800">' for url in detail_img[2:-1]
            )
    ...
    def add_image_watermark(
        self, original_image, watermark_image_path, image_scale=0.2, margin=3
    ):
        """
        给图片添加水印（右上角，可调整边距）
        参数:
