...
class TaoBao(BaseCrawler):
    ...
    def get_main_images(self):
        ...
        main_images = [
            ...
            for url in self.data["data"]["componentsVO"]["headImageVO"]["images"][:8]
        ]
        return main_images
    ...
    def get_sku_price(self, specifications, skus=None, vid=None, pid=None):
        ...
        for sku in skus:
            ...
            if specifications == 1 and vid not in sku["propPath"]:
                continue
            if specifications == 2 and (
                vid not in sku["propPath"] or pid not in sku["propPath"]
            ):
                continue
            # 获取 sku_id
    ...
    def get_stock_data(self, sku_id):
        """获取sku库存"""
        stock_data = self.data["data"]["skuCore"]["sku2info"][str(sku_id)][
            "quantityText"
        ]
        ...
        for i in res["infos"]:
            if i["title"] == "参数":
                attribute_list = [
                    (f"{x['title']} : {x['text'][0]}")
                    for x in i["items"]
                    if all(word not in x["title"] for word in Filter_words)
                ]
    ...
    def build_product_package(self):
        ...
        if specifications == 0:
            ...
            sku_assembly = {
                "sku_data": {
                    "price": self.get_sku_price(specifications),
                    "stock": stock,
                }
            }
        else:
            ...
            if specifications == 1:
                ...
                for i in props[0]["values"]:
                    for j in props[1]["values"]:
                        price = self.get_sku_price(
                            specifications, skus_data, i["vid"], j["vid"]
                        )
                        if price is None:
                            continue
                        skus = {
