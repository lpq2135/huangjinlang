...
class Ruten(BaseCrawler):
    ...
    def get_description(self):
        ...
        # 请求详情链接获取 html 数据
        html_data = self.request_function(
            description_url, headers=headers, proxies=self.proxies
        ).text
        # 格式化 html 数据
        if html_data:
            try:
    ...
    def get_ids_by_reception(self, keywords):
        ...
        discountprice_result = {
            i["ProdId"]: i["Discount"][0]["Price"]
            for i in discountprice_data
            if len(i["Discount"]) != 0
        }
    ...
    def get_ids_by_discountprice(self, v3_ids):
        ...
        discountprice_result = self.request_function(
            discountprice_url, headers=self.headers
        ).json()
    ...
    def build_product_package(self):
        ...
        if not self.product_data["item"]["specInfo"]:
            ...
            sku_assembly = {
                "sku_data": {
                    "price": self.product_data["item"]["directPrice"],
                    "stock": self.product_data["item"]["remainNum"],
                }
            }
        else:
            # 规格数
