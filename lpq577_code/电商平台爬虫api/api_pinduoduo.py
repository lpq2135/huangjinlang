...
class PinDuoDuo(BaseCrawler):
    ...
    def get_product_attribute(self):
        ...
        # 若详情文字的标题包含过滤词则过滤
        description = [f"{i['key']} : {i['values'][0]}" for i in goods_property if not any(filter_word in i['key'] for filter_word in Filter_words)]
        return description
    ...
    def build_product_package(self):
        ...
        if specifications == 1:
            ...
            for i in skus_data:
                ...
                skus = {
                    'remote_id': self.product_id + '_' + self.generate_random_string(10),
                    'name': i['specs'][0]['spec_value'] + '||' + i['specs'][1]['spec_value'],
                    'imageUrl': i.get('thumbUrl', None),
                    'price': i['normalPrice'],
                    'stock': stock
                }
            ...
            return {'platform': 'pinduoduo', 'code': 5, 'message': 'sku规格数超出', 'product_id': self.product_id}
        ...
        return {'platform': 'pinduoduo', 'code': 0, 'message': '请求成功', 'data': product_package}
