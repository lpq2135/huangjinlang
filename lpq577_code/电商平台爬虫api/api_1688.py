...
class BaseCrawler:
    """所有爬虫类的基类，包含共用方法"""
    def request_function(self, url, method='GET', headers=None, max_retries=5):
        """通用的请求方法"""
        # 进行最多max_retries次重试
        ...
        logging.error(f"请求失败，已尝试{max_retries}次: {url}")
        return None
...
class Alibaba(BaseCrawler):
    ...
    def get_title(self):
        """获取标题并进行处理"""
        title_word_replacement = ['厂家', '供应', '销售', '规格', '齐全', '现货', '发货', '促销', '生产', '东莞', '武汉', '地区', '成都', '跨境', '批发', '直发', '量大从优']
    ...
    def get_detail_images(self):
        ...
        if data and 'detailUrl' in data:
            # 请求 detailUrl 获取详情数据
            detail_data = self.request_function(data['detailUrl'], headers=self.headers).text
    ...
    def build_product_package(self):
        ...
        if self.source:
            ...
            if len(main_images) == 0:
                return {'platform': '1688', 'code': 3, 'message': '主图异常', 'product_id': self.product_id}
            if len(main_images) != 0:
                # 获取 skuProps 的参数
                ...
                if not skuProps:
                    ...
                    if 'skuRangePrices' not in self.source['globalData']['orderParamModel']['orderParam']['skuParam']:
                        return {'platform': '1688', 'code': 4, 'message': '商品数据包异常', 'product_id': self.product_id}
                ...
                # 规格数为 2
                    ...
                    return {'platform': '1688', 'code': 5, 'message': 'sku规格数超出', 'product_id': self.product_id}
                # 组装数据包
                product_package = {
                ...
                return {'platform': '1688', 'code': 0, 'message': '请求成功', 'data': product_package}

        else:
            return {'platform': '1688', 'code': -1, 'message': '商品下架或离线', 'product_id': self.product_id}

class TaoBao(BaseCrawler):
    pass

class TMall(BaseCrawler):
    pass
