        ...
        try:
            if self.source:
                ...
                if title_data['status']:
                    ...
                    return {'status': False, 'specifications': None, 'data': f"商品标题包含违禁词【{title_data['data']}】"}

            else:
                return {'status': False, 'specifications': None, 'data': '商品已离线'}
        except Exception:
            return {'status': False, 'specifications': None, 'data': '商品未知异常'}


if __name__ == '__main__':
    res = Alibaba('1019312648')
    try:
