...
class RakutenProduct(BaseCrawler):
    ...
    def create_product(self):
        ...
        data = {
            ...
            'productListedShops': {
                ...
                'title': {
                    'zh_TW': 'Title Variant_Product'
                },
                'description': {
                    'zh_TW': 'Description Variant_Product'
                },
                'marketplaceRakutenCategoryId': '10009',
                'variantInfos': {
                    'Variant_Product_Blue': {
                        'pricing': {
                            'price': 123
                        }
                    },
                    'Variant_Product_Yellow': {
                        'pricing': {
                            'price': 123
                        }
                    }
    ...
    def create_shop_categories(self):
        ...
        response = self.request_function(url, 'post', headers=self.headers, data=json.dumps(data)).json()
        return response
...
if __name__ == '__main__':
    rakuten_istances = RakutenProduct('luinte', 'NnZqTGg3dERQVkxWaTN5ZDpIWDlzeGU5YkNUYkQyNTRv')
