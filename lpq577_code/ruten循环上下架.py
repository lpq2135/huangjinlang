...
def record_product_id_listing_status(store, product_id, g_no):
    try:
        ...
        # 如果发生异常，记录错误日志并回滚事务
        logging.warning(
            "记录商品成功上传信息异常, store: %s, product_id: %s, error: %s",
            store,
            product_id,
            str(e),
        )
...
def unable_to_list_id_record(store, product_id):
    try:
        ...
        # 如果发生异常，记录错误日志并回滚事务
        logging.warning(
            "记录商品上传失败信息异常, store: %s, product_id: %s, error: %s",
            store,
            product_id,
            str(e),
        )
...
def record_main_image_error(store, product_id):
    try:
        ...
        # 如果发生异常，记录错误日志并回滚事务
        logging.warning(
            "记录上传失败的商品ID异常, store: %s, product_id: %s, error: %s",
            store,
            product_id,
            str(e),
        )
...
def record_category_id(category_id, is_valid):
    try:
        ...
        # 如果发生异常，记录错误日志并回滚事务
        logging.warning(
            "记录类目id信息异常, category_id: %s, error: %s", category_id, str(e)
        )
        cnx.rollback()  # 回滚事务，避免无效或部分更新的数据
...
def get_category_id_status(category_id):
    try:
        ...
        # 如果发生异常，记录错误日志并回滚事务
        logging.warning(
            "获取类目id状态异常, category_id: %s, error: %s", category_id, str(e)
        )
...
def modify_product_status(store, remove_id):
    try:
        ...
        # 如果发生异常，记录错误日志并回滚事务
        logging.warning(
            "更新商品下架信息异常, store: %s, product_id_list: %s, error: %s",
            store,
            remove_id,
            str(e),
        )
...
def get_the_daily_listing_quantity(store):
    try:
        ...
        current_time = datetime.now().strftime("%Y-%m-%d")
        query = (
            f"SELECT COUNT(*) FROM {table_name} WHERE store = %s AND update_time = %s"
        )
        cursor.execute(
            query,
            (
        ...
        logging.warning(
            "获取当前上货数量异常, store: %s, current_time: %s",
            store,
            current_time,
            str(e),
        )
...
# 获取product_id数据库状态
def get_product_id_status(store, product_id):
    try:
        cnx, cursor = mysql_pool.get_conn()
        table_name = f"cyclic_on_and_off_record_by_{store}"  # 构建完整的表名
        query = f"SELECT COUNT(*) FROM {table_name} WHERE product_id = %s"
        cursor.execute(query, (product_id,))
        row_count = cursor.fetchone()
        return row_count[0]
    except Exception as e:
        logging.warning(
            "获取product_id数据库状态异常, store: %s, product_id: %s",
            store,
            product_id,
            str(e),
        )
...
class RutenUpload(BaseCrawler):
    def __init__(
        ...
        if self.proxies:
            if self.get_payment_and_shipping_old(self.store) is False:
    ...
    def check_watermark_image_exists(self):
        ...
        dev_path = rf"C:\Users\Administrator\Desktop\水印图片测试\店铺水印\LOGO\{watermark_filename}"
    ...
    # 获取店铺对应的付款和运输方式
    def get_payment_and_shipping_old(self, store):
        try:
            cnx, cursor = mysql_pool.get_conn()
            cursor.execute(
        ...
        try:
            ...
            if self.is_save_img == 1:
                ...
                if self.img_save_path:
                    ...
                    logging.info(
                        f"{self.store}-{product_id}-图片已保存到: {original_filepath}"
                    )

            # 3. 判断是否需要水印
            need_watermark = self.watermark_image_path is not None and (
        ...
        for num in range(6):
            ...
            if response.status_code == 200:
                try:
                    ...
                    id_list = tree.xpath(
                        '//tr[@class="row-odd" or @class="row-even"]/@data-gno'
                    )
                    if len(id_list) == 0:
                        continue
                    new_list = []
                    for i in id_list:
                        product_data = {
                            "product_id": i,
                            "title": tree.xpath(
                                f'//tr[@data-gno="{i}"]/td[3]/a/text()'
                            )[0],
                            "min_price": tree.xpath(
                                f'//tr[@data-gno="{i}"]/td[4]/div/@data-price-min'
                            )[0],
                            "max_price": tree.xpath(
                                f'//tr[@data-gno="{i}"]/td[4]/div/@data-price-max'
                            )[0],
                            "stock": tree.xpath(f'//tr[@data-gno="{i}"]/td[5]/text()')[
                                0
                            ]
                            .replace("(", "")
                            .strip(),
                            "sales": tree.xpath(
                                f'//tr[@data-gno="{i}"]/td[6]/a/text()'
                            )[0],
                            "click": tree.xpath(f'//tr[@data-gno="{i}"]/td[7]/text()')[
                                0
                            ],
                            "cart": tree.xpath(f'//tr[@data-gno="{i}"]/td[8]/text()')[
                                0
                            ],
                            "upduct_time": tree.xpath(
                                f'//tr[@data-gno="{i}"]/td[10]/text()'
                            )[0].strip(),
                        }
                    ...
                    logging.warning(
                        f"{self.store}-获取第{page}页商品信息失败, 错误信息：{e}"
                    )
                    time.sleep(2)
        return None
    ...
    def process_category_id(self, category_id):
        ...
        category_response = self.request_function(
            get_category_url, headers=self.headers
        ).json()
        ...
        regulations_response = self.request_function(
            regulations_url, headers=self.headers
        ).json()
        if (
            regulations_response["data"]["bsmi_status"] == 1
            or regulations_response["data"]["ncc_status"] == 1
        ):
            record_category_id(category_id, "0")
            return "00090014"
    ...
    def get_class(self, text):
        ...
        response2 = self.request_function(
            url2, "post", headers=self.headers, data=data2
        ).json()
    ...
    def process_user_class(self, product_id):
        ...
        for num in range(6):
            response = self.request_function(
                url, "post", headers=self.headers, data=data
            )
            if response.status_code == 400:
                continue
            if response.status_code == 200:
                try:
                    tree = html.fromstring(response.text)
                    try:
                        user_class_select = tree.xpath("//option[@selected]/@value")[0]
                    except IndexError:
                        user_class_select = "0"
                    # 获取表单数据
                    data = {
                        ...
                        "time_key": "",
                        "g_no_0": product_id,
                        f"hash_{product_id}": tree.xpath(
                            '//input[@id="g_hash0"]/@value'
                        )[0],
                        f"g_mode_{product_id}": tree.xpath(
                            '//input[@id="g_mode0"]/@value'
                        )[0],
                        f"g_sold_num_{product_id}": tree.xpath(
                            '//input[@id="g_sold_num0"]/@value'
                        )[0],
                        f"g_ori_price{product_id}": tree.xpath(
                            '//input[@id="g_ori_price0"]/@value'
                        )[0],
                        f"g_class_{product_id}": tree.xpath(
                            '//input[@id="g_class0"]/@value'
                        )[0],
                        f"g_name_{product_id}": tree.xpath(
                            '//input[@id="g_name0"]/@value'
                        )[0],
                        f"user_class_select_{product_id}": user_class_select,
                        f"g_num_{product_id}": tree.xpath(
                            '//input[@id="g_num0"]/@value'
                        )[0],
                        f"g_direct_price2_{product_id}": tree.xpath(
                            '//input[@id="g_direct_priceB0"]/@value'
                        )[0],
                    }
                    return data
                except Exception as e:
                    logging.warning(
                        f"{self.store}-{product_id}-获取后台自定义分类失败, 错误信息: {e}"
                    )
        return None
    ...
    def product_items_v2(self, product_id):
        url = (
            f"https://rapi.ruten.com.tw/api/items/v2/list?gno={product_id}&level=simple"
        )
        headers = {
        ...
        try:
            ...
            logging.warning(
                f"{self.store}-{product_id}-请求商品V2接口失败(更换产品接口)-{e}"
            )
            result2 = self.get_product_package(product_id)
            return not result2["item"]["isSoldEnd"]
    ...
    def get_product_package(self, product_id):
        ...
        if match:
            try:
                ...
                logging.warning(
                    f"{self.store}-{product_id}-获取商品数据包失败, 错误信息: {e}"
                )

    # 检查标题是否修改成功
    def title_check(self, product_id, title):
        url = (
            f"https://rapi.ruten.com.tw/api/items/v2/list?gno={product_id}&level=simple"
        )
        headers = {
        ...
        try:
            result1 = self.request_function(url, headers=headers).json()
            gno_name = result1["data"][0]["name"]
            if gno_name == title:
                return True
            else:
                time.sleep(12)
                result2 = self.get_product_package(product_id)
                gno_name = result2["item"]["name"]
                if gno_name == title:
                    return True
                return False
        except Exception as e:
    ...
    def single_remove_products(self, product_id):
        ...
        data = {"type": "down", "gno_list": product_id}
        try:
            response = self.request_function(
                url, "post", headers=self.headers, data=data, proxies=self.proxies
            ).json()
    ...
    def batch_remove_products(self, remove_list):
        ...
        data = {"type": "down", "gno_list": gno_list}
        try:
            response = self.request_function(
                url, "post", headers=self.headers, data=data, proxies=self.proxies
            ).json()
    ...
    # 修改标题重新上架(大量修改入口)
    def modify_title_to_upload(self, product_id, g_no, title):
        ...
        response1 = self.request_function(
            url1, "post", headers=self.headers, data=data, proxies=self.proxies
        )
        time.sleep(2)
        if "filter_msg" in response1.url:
            logging.warning(
                f"{self.store}-{g_no}-无法通过修改标题的方式上架(原始id-{product_id})"
            )
            return {"code": 4}
            ...
            if match:
                ...
                response2 = self.request_function(
                    url2, headers=self.headers, proxies=self.proxies
                )
                if "1件商品修改成功！" in response2.text:
                    title_check = self.title_check(g_no, title)
                    if title_check:
                        logging.info(
                            f"{self.store}-{g_no}-标题更新成功(原始id-{product_id})"
                        )
                        return {"code": 0}
                    logging.info(
                        f"{self.store}-{g_no}-标题更新失败(原始id-{product_id})"
                    )
                    return {"code": 4}
            logging.warning(
                f"{self.store}-{g_no}-无法通过修改标题的方式上架(原始id-{product_id})"
    ...
    def price_conversion(self, data, price_multi, price_add):
        ...
        if specifications == 0:
            data["data"]["skumodel"]["sku_data"]["price"] = int(
                float(data["data"]["skumodel"]["sku_data"]["price"])
                * float(price_multi)
                + float(price_add)
            )
        else:
            for i in data["data"]["skumodel"]["sku_data"]["sku_parameter"]:
                i["price"] = int(
                    float(i["price"]) * float(price_multi) + float(price_add)
                )
        return data
    ...
    def structure_sku_data(self, product_data):
        ...
        # 规格等于 0
            ...
            stock = int(product_data["data"]["skumodel"]["sku_data"]["stock"])
            show_num = stock if 150 <= stock <= 999 else 499
    ...
    def cycle_on_and_off_shelves(self, product_id):
        # 判断上品的状态（前置条件）
        product_id_mysql_count = get_product_id_status(self.store, product_id)
        if product_id_mysql_count != 0:
        ...
        if (
            "本店支持開發票及信用卡支付，若需要使用信用卡付款請聯絡客服" in html_old
            or account != "huangjinlang"
        ):
            html_new = html_old
        else:
        ...
        if upload_products["code"] == 0:
            ...
            logging.info(
                f"{self.store}-当日商品已上传成功{self.upload_count}-本轮商品已成功上传{self.initial_count}"
            )
    ...
    def upload_products(self, product_id, upload_product_package):
        ...
        while True:
            ...
            response_initial = self.request_function(
                url_initial, headers=self.headers, proxies=self.proxies
            )
        ...
        for idx, i in enumerate(upload_product_package["main_images"]):
            ...
            if result_dict["code"] == 0:
                ...
                if result["complete"] and result["content"]["file_name"] != "":
                    image = {
                        "img_name": result["content"]["file_name"],
                        "storage": result["content"]["storage"],
                    }
        ...
        # 标题
        title = upload_product_package["title"][:60]
        ...
        data_dict = {
            ...
            "g_name": title,  # 标题
            "user_class_select": upload_product_package[
                "user_class_select"
            ],  # 后台自自定义分类编码
            "g_mode": "B",  # mode
            "g_direct_price": upload_product_package["g_direct_price"],
            "spec_info": (
                json.dumps(upload_product_package["spec_info"])
                if upload_product_package["specifications"] != 0
                else ""
            ),  # sku数据包
        ...
        for retry in range(2):
            ...
            response_action = self.request_function(
                url_action,
                "post",
                headers=headers,
                data=multipart_data,
            ...
            if "filter_msg" in response_action_url:
                logging.warning(
                    f"{self.store}-{product_id}-此商品无法上架露天(尝试修改标题的方式上架)"
                )
                data_dict["g_name"] = f"huangjinlang_{self.generate_random_string()}"
        ...
        response_finalize = self.request_function(
            url_finalize, headers=self.headers, proxies=self.proxies
        )
        ...
        if "g_no" in response_finalize_url:
            ...
            if modify_title_mark:
                filter_data = self.modify_title_to_upload(product_id, g_no, title)
...
def process_store(row):
    try:
        ...
        max_number_of_listings_per_day = int(row[9])  # 每日最大上架数

        # 获取当前上货数量
        upload_count = get_the_daily_listing_quantity(store)
        if upload_count >= max_number_of_listings_per_day:
            return
        ...
        while True:
            ...
            product_id_list = ruten_instance.get_pagination_product_id(
                page_num
            )  # 获取分页链接的商品数据
            ...
            with concurrent.futures.ThreadPoolExecutor(
                ...
                futures = []
                for product_data in product_id_list:
                    ...
                    # 判断标题空格数
                    if (
                        title.count(" ") <= number_of_spaces_in_title
                        and int(product_data["sales"]) <= maximum_bids
                    ):
                        # 判断商品价格最小值、最大值、出价数、点阅数、追踪数
                        if (
                            int(product_data["min_price"]) < min_price
                            and int(product_data["max_price"]) > 1000
                        ) or (min_price <= int(product_data["min_price"]) <= max_price):
                            if (
                                int(product_data["click"]) <= maximum_views
                                and int(product_data["cart"]) <= maximum_traces
                            ):
                                # 满足条件，提交任务到线程池
                                futures.append(
                                    executor.submit(
                                        ruten_instance.cycle_on_and_off_shelves,
                                        product_id,
                                    )
                                )
                            else:
                                logging.info(
                                    f"{store}-{product_id}-不做循环上下架处理(点阅数/追踪数)"
                                )
                        else:
                        ...
                        logging.info(
                            f"{store}-{product_id}-不做循环上下架处理(标题空格数/销量)"
                        )
...
if __name__ == "__main__":
    ...
    if account == "yey":
        mysql_pool = MySqlPool(
            host="db-7ff794f7008e4a0595ac3ab2f02bca6c.mysql.rds.aliyuncs.com",
            password="Luteen123@",
            user="administrator",
            database="luteen",
        )
        ...
        mysql_pool = MySqlPool(
            host="rm-gw80g135076f1osh41o.mysql.germany.rds.aliyuncs.com",
            password="Qiang123@",
            user="huangjinlang",
            database="ruten_upload",
        )
        proxies = {
        ...
        mysql_pool = MySqlPool(
            host="rm-gw894lxac3o2j055k0o.mysql.germany.rds.aliyuncs.com",
            password="Wait0you",
            user="yihe",
            database="ruten_yihe",
        )
        proxies = {
            "http": "http://brd-customer-hl_34914aa7-zone-datacenter_proxy2:85olh0om37b9@brd.superproxy.io:33335",
            "https": "https://brd-customer-hl_34914aa7-zone-datacenter_proxy2:85olh0om37b9@brd.superproxy.io:33335",
        }
    else:
        # 异常退出，状态码为 1
        sys.exit(1)
    ...
    logging.info(f"当前店铺执行线程{max_workers}")
