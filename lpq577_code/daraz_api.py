...
class DarazBase:
    ...
    def __init__(self):
        ...
        self.app_key = '502742'
        self.app_secret = '0XGyiUMf0obAP9FueDD16fid4M5xgmaV'
...
class DarazProduct(Helper, DarazBase):
    """
    daraz上货处理的相关代码
    """
...
class DarazPrice:
    ...
    daraz 价格计算
    """
    def calculate_price_percentage(self, sku_parameter, price_max):
        """
        计算超出价格上限得百分比
        """
        price_exceeds_percentage = len(list(filter(lambda x: x['price'] > price_max, sku_parameter))) / len(
            sku_parameter)
        return price_exceeds_percentage

    def processing_price(self, upload_site, data, weight):
        """
        计算站点的价格是否符合要求
        """
        if 'sku_parameter' in data['sku_data']:
            ...
            for i in sku_parameter:
                i['price'] = self.calculate_local_price(upload_site, i['price'], weight)
            if upload_site == 'pk':
                pk_price_exceeds_percentage = self.calculate_price_percentage(sku_parameter, 4600)
            ...
            pk_price_0 = self.calculate_local_price(upload_site, data['sku_data']['price'], weight)
...
class DarazMessages(DarazBase, Helper):
...
if __name__ == '__main__':
    daraz_messages = DarazMessages('pk', '50000601b02rhS10db5a6apebgBRfKyzCmiLiahszDcblSxfQQ3SpMywbjtI8j')
    get_session_list = daraz_messages.get_session_list()
